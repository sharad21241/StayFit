//
//  MBLCAnimatedPageControl.m
//  MBLCAnimatedPageControl
//
//

//This class is a custom class for Page Controller

#import "MBLCAnimatedPageControl.h"
#import "MBIndicatorView.h"

static CGFloat kLCDoubleNumber = 2.0f;
static CGFloat kLCHalfNumber = 0.5f;
static CGFloat kLCMultiple = 1.4f;

@interface MBLCAnimatedPageControl ()<UIScrollViewDelegate>
...
@implementation MBLCAnimatedPageControl
...
/**
 ...
 @param indicatorDiameter diameter
 */
...
- (void)addIndicatorsWithIndex:(NSInteger)index{
    for (NSInteger number = index; number < _numberOfPages; number ++ ) {
        ...
        if (_pageStyle == LCFillColorPageStyle) {
            indicator = [[MBIndicatorView alloc] init];
            [(MBIndicatorView *)indicator backView].backgroundColor = _currentPageIndicatorColor;
            [(MBIndicatorView *)indicator frontView].backgroundColor = _pageIndicatorColor;
            [(MBIndicatorView *)indicator backView].layer.cornerRadius = _radius;
            [(MBIndicatorView *)indicator frontView].layer.cornerRadius = _radius;
            [self configZeroScaleAnimation:[(MBIndicatorView *)indicator backView]];
            [self configZeroScaleAnimation:[(MBIndicatorView *)indicator frontView]];
        }
        else{
...
- (void)configCurrentIndicator{
    ...
    if ([view isKindOfClass:[MBIndicatorView class]]) {
        [(MBIndicatorView *)view frontView].layer.timeOffset = 1.0f;
    }
    else{
        view.layer.timeOffset = 0.0f;
    }
...
- (void)configDefaultIndicator{
    ...
    if (self.indicatorViews.count) {
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            CGFloat rate = 0;
            if (_scrollStyle == Carousel)
            {
                CGFloat offset = ((iCarousel *)self.sourceScrollView).scrollOffset;
                rate = offset / ((iCarousel *)self.sourceScrollView).bounds.size.width;
            }
            else if (_scrollStyle == CollectionView)
            {
                CGPoint offset = ((UIScrollView *)self.sourceScrollView).contentOffset;
                rate = offset.x / ((UIScrollView *)self.sourceScrollView).bounds.size.width;
            }
            ...
            if (_pageStyle == LCScaleColorPageStyle) {
                ...
                [(MBIndicatorView *)currentIndicator frontView].layer.timeOffset = 1.0f;
            }
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context{
    ...
    if (rate >= 0.0f && rate <= _numberOfPages - 1) {
        ...
        if (_scrollStyle == Carousel)
        {
            if (!((iCarousel *)self.sourceScrollView).decelerating && _isDefaultSet) {
                return;
            }
        }
        else if (_scrollStyle == CollectionView)
        {
            if (!((UIScrollView *)self.sourceScrollView).decelerating && _isDefaultSet) {
                return;
            }
        }
        ...
        if (_pageStyle == LCScaleColorPageStyle) {
            ...
            [(MBIndicatorView *)currentPointView frontView].layer.timeOffset = timeOffset;
            [(MBIndicatorView *)lastPointView frontView].layer.timeOffset = 1.0f - timeOffset;
        }
        self.currentPage = currentIndex;
    }
}
...
- (void)changePage:(NSInteger)current_Index{
    self.currentPage = current_Index;
    NSInteger i = 0;
    for (UIView *view in _indicatorViews) {
        
        if (self.currentPage == i)
        {
            view.layer.timeOffset = 1.0f;
        }
        else
        {
            view.layer.timeOffset = 0.0f;
